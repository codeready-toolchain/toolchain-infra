...
function deploy_operators() {
  if [[ ${CLUSTER_TYPE} == "host" ]]; then
    ...
    REGISTRATION_SERVICE_URL=$REGISTRATION_SERVICE_URL CONSOLE_NAMESPACE=$CONSOLE_NAMESPACE CHE_NAMESPACE=$CHE_NAMESPACE CHE_NAMESPACED_NAME=$CHE_NAMESPACED_NAME NAMESPACE=$HOST_OPERATOR_NS envsubst < ./config/host_operator_config.yaml | oc apply -f -
